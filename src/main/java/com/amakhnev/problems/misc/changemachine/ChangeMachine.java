...
public class ChangeMachine {
  ...
  /**
   * This function takes a list of coins in the till and return the correct coins which adds to
   * target value.
   *
   * @param coins list of coins in UK denomination.
   * @param value - target value to achieve in pence.
   ...
   */
  public List<Integer> change(List<Integer> coins, int value) {
    List<Integer> till = new ArrayList<>(coins);
    if (value <= 0) {
      throw new IllegalArgumentException("Value (" + value + ") should be positive.");
    }
    ...
    for (int num :
        till) { // check if all coins are coins indeed. Can delete from till, but follow Fail-Fast
                // approach.
      UKCoin.getByValue(num);
    }
    // need to sort the till for
    ...
    for (int i = 1; i < till.size(); i++) {
      for (int j = 1; j < value + 1; j++) {
        solvable[i][j] =
            solvable[i - 1][j] || ((till.get(i) <= j) && solvable[i - 1][j - till.get(i)]);
      }
    }
  ...
  private void backtrack(
    ...
    if (target >= till.get(numIdx)
        && solvable[numIdx - 1][
            target - till.get(numIdx)]) { // there is a possible path which includes current item
      path.add(till.get(numIdx));
  ...
  public List<Integer> changeNaive(List<Integer> tillList, int value) {
    ...
    EnumMap<UKCoin, Integer> till = new EnumMap<>(UKCoin.class);
  ...
  private List<Integer> getCoinsFor(
      EnumMap<UKCoin, Integer> till, int value, List<Integer> coinsUsed) {
